...
xlsx_path = "/DVS/results/DVS_LA_Capabilities2.xlsx"
...
def collect_all_capability_realizations():
    """Collect all existing capability realizations in the model"""
    realizations = {}

    try:
        print("Starting to collect all existing capability realizations...")
        ...
        for cr in java_pkg.getOwnedCapabilityRealizations():
            ...
            print(f"Found capability realization: {cr.getName()} (ID: {cr_id})")

        total_count = len(realizations)
        print(f"Found {total_count} existing capability realizations")
        return realizations
    except Exception as e:
...
def delete_unused_capability_realizations(imported_realizations):
    """Delete capability realizations that exist in the model but were not imported"""
    try:
        deleted_count = 0

        print("Starting to delete unused capability realizations...")
        ...
        # Get all capability realizations in the package
        java_pkg = get_java_object(capability_realization_pkg)
        for cr in list(java_pkg.getOwnedCapabilityRealizations()):
            cr_id = cr.getSid()
            ...
            print(f"Deleting capability realization: '{cr_name}' (ID: {cr_id})")
            EObject.delete_e_object(CapabilityRealization(cr))
            deleted_count += 1

        print(f"Deleted {deleted_count} unused capability realizations")
...
try:
    ...
    print("Collecting existing capability realizations...")
    existing_realizations = collect_all_capability_realizations()

    # Dictionary to store imported capability realizations and their involvements
    ...
    for row in ws.iter_rows(min_row=2):
        try:
            ...
            if existing_cr:
                print(f"Capability realization with ID {cr_id} and name '{cr_name}' already exists.")
            ...
            for i in range(max_involved_components):
                ...
                involvements = collect_capability_involvements(cr)
                imported_realizations[cr_id] = {
                    'realization': cr,
                    'involvements': involvements
                }

            print(f"Successfully imported capability realization: {cr_name}")
    ...
    print("\nDeleting unused capability realizations...")
    deleted_count = delete_unused_capability_realizations(imported_realizations)
    print(f"Deleted {deleted_count} capability realizations that were not in the import file")
