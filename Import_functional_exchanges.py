...
Usage:
  python import_functional_exchanges_from_excel.py [input.xlsx] [functions.sysml]
...
def parse_id_to_def_name(sysml_text: str) -> Dict[str, str]:
    ...
    Handles deduplicated names like ParticipateInLesson2 that differ from
    the raw Excel-derived name.
    """
...
def _assign_conn_iface_names(
    ...
    for exch in exchanges:
        from_type = id_to_def_name.get(exch.from_action_id, to_type_name(exch.from_action_name))
        to_type   = id_to_def_name.get(exch.to_action_id,   to_type_name(exch.to_action_name))
...
def _build_action_ports(
    ...
    for exch in exchanges:
        ...
        for action_id, usage, def_name, port_id in entries:
            if action_id not in action_ports:
                action_ports[action_id] = []
...
def inject_exchanges(sysml_text: str, exchanges: List[FunctionalExchange]) -> str:
    ...
    action_ports     = _build_action_ports(exchanges, registry)
